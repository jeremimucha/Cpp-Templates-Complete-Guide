...
{
private:
    std::unique_ptr<FunctorBridge<R, Args...>> bridge_{nullptr};
public:
    ...
    {
        if (other.bridge_) {
            bridge_.reset(other.bridge_->clone());
        }
    }
    ...
    friend bool operator==(FunctionPtr const& lhs, FunctionPtr const& rhs)
    {
        ...
        return lhs.bridge_.get()->equals(rhs.bridge_.get());
    }
